...
export function SandboxNoticeToast({
  ...
  onAction,
  displayNumber,
  lockUntilAction = true,
}: SandboxNoticeToastProps) {
  useEffect(() => {
    if (!open) return;
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") {
        if (lockUntilAction) return;
        onClose();
      }
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [open, onClose, lockUntilAction]);
    ...
    <>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-6 z-50 flex justify-center px-4 sm:bottom-10">
        <div className="pointer-events-auto relative w-full max-w-md overflow-hidden rounded-3xl border border-primary/50 bg-[#0b0a1c]/95 p-6 shadow-[0_30px_70px_rgba(88,51,255,0.55)] backdrop-blur-xl sm:rounded-[36px] sm:p-8">
          <div
            className="pointer-events-none absolute -inset-20 bg-[radial-gradient(circle_at_top,_rgba(99,102,241,0.28),transparent_60%)]"
            aria-hidden
          />
          <div className="relative space-y-6">
            ...
            <div className="space-y-3 text-center">
              ...
              <h3 className="text-xl font-bold text-foreground sm:text-2xl">
                Klik & Kirim Pesan WhatsApp Terlebih Dahulu
              </h3>

              <p className="text-sm text-muted-foreground">
                Sistem WhatsApp kami masih mode sandbox. Agar <strong>notifikasi transaksi</strong> (OTP & update order) bisa terkirim,
                Anda perlu <strong>memulai chat</strong> ke nomor <span className="font-semibold text-primary">{displayNumber}</span> terlebih dahulu.
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm text-muted-foreground shadow-[0_0_30px_rgba(76,29,149,0.35)]">
              <p className="mt-1">1) Klik tombol di bawah untuk membuka WhatsApp.</p>
              <p className="mt-1">
                2) <strong>Kirim pesan apa saja</strong> (mis. “Halo admin”) ke nomor tersebut agar nomor Anda
                <strong> tersambung</strong> dan siap menerima notifikasi.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:items-center sm:justify-between">
              ...
              <p className="text-[11px] text-muted-foreground sm:ml-2">
                *Tombol akan membuka WhatsApp. Setelah mengirim pesan, notifikasi akan aktif.
              </p>
            </div>
          </div>
        </div>
